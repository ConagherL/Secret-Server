...
- **Two versions of this code exist. The first version handles the RPC for App registration and AKV secrets. The second one utilizes the released AKV external secrets feature in Secret Server and only performs the RPC on the app registration. External secrets must be set up and listed on the master secret.
