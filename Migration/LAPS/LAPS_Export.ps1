...
#Write report header
$strToReport = "ComputerName" + $strDelimiter + "OperatingSystem" + $strDelimiter + "Password" + $strDelimiter + "ExpTime" + $strDelimiter + "DistinguishedName" + $strDelimiter + "Object Status" + $strDelimiter + "Last Login"
...
foreach ($Computer in $Computers) {
    ...
    if ($Computer.'ms-Mcs-AdmPwd'){
        ...
        $strlastLogonTimestamp = $Computer.'lastLogonTimestamp'

        if ($strlastLogonTimestamp -ge 0) {$strlastLogonTimestamp = $([datetime]::FromFileTime([convert]::ToInt64($strlastLogonTimestamp)))}

        $strlastLogonTimestamp = "{0:yyyy-MM-dd HH:mm:ss}" -f [datetime]$strlastLogonTimestamp

        $strToReport = $Computer.Name + $strDelimiter + $Computer.OperatingSystem + $strDelimiter + """$strComputerPassword""" + $strDelimiter + """$strComputerExpTime""" + $strDelimiter + $Computer.DistinguishedName + $strDelimiter + $Computer.Enabled + $strDelimiter + """$strlastLogonTimestamp"""
