# --- CONFIGURATION ---
$BaseUrl        = "https://XXXX.secretservercloud.com"                    # Change this to your Secret Server URL
$TokenUrl       = "$BaseUrl/oauth2/token"                                # OAuth2 token endpoint
$FixDuplicates  = $true                                                  # Enable renaming of secrets with duplicate names
$FixWhitespace  = $false                                             # Enable trimming whitespace from folder/secret names

# NEW: Choose whether to import duplicates from CSV
$UseCsvForDuplicates = $false                                           # Set to $true to import instead of discover
$CsvDuplicatePath     = "C:\temp\DuplicateSecrets.csv"                  # Path to CSV of duplicates
...
function Update-SecretName {
    ...
    # NEW: Guard against empty/whitespace names (prevents "Invalid Secret Name")
    if ([string]::IsNullOrWhiteSpace($NewName)) {
        Write-Log "[WARNING] Skipping update for ID=$SecretId because new name is empty or whitespace" "WARNING"
        return
    }
...
function Find-DuplicateSecrets {
    ...
    foreach ($group in $nameGroups) {
        ...
        for ($i = 1; $i -lt $groupSorted.Count; $i++) {
            ...
            # NEW: Guard against null/empty names produced by bad data
            if ([string]::IsNullOrWhiteSpace($newName)) {
                Write-Log "[WARNING] Skipping Secret ID=$($secret.id) because computed new name is empty/invalid" "WARNING"
                continue
            }
...
# --- IMPORT DUPLICATES FROM CSV (ENHANCED WITH AUTO-NAMING) ---
function Import-DuplicateSecrets {
    param (
        [string]$CsvPath,
        [string]$AuthToken
    )
    ...
    if (-not (Test-Path $CsvPath)) {
        Write-Log "CSV import selected, but file not found at: $CsvPath" "ERROR"
        return
    }

    Write-Log "Importing duplicate secrets from: $CsvPath"

    try {
        $csvData = Import-Csv -Path $CsvPath
        $total = $csvData.Count
        Write-Log "Loaded $total duplicate records from CSV"

        # Build a map of counts by Secret Name for suffixing
        $nameCounts = @{}
        foreach ($row in $csvData) {
            $baseName = $row.'Secret Name'
            if (-not $nameCounts.ContainsKey($baseName)) { $nameCounts[$baseName] = 0 }
            $nameCounts[$baseName]++
            $suffix = $nameCounts[$baseName]
            $newName = if ($suffix -gt 1) { "$baseName-$suffix" } else { $baseName }

           # Use quoted property names so headers with spaces resolve correctly
            $secretId   = [int]$row.'SecretID'
            $folderPath = $row.'Folder Path'

        Update-SecretName -SecretId $secretId -OldName $baseName -NewName $newName `
            -FolderPath $folderPath -AuthToken $AuthToken -Reason "Duplicate Fix"
        }

        Write-Log "Processed $total imported duplicate secrets"
    }
    catch {
        Write-Log "[ERROR] Failed to import or process CSV file: $_" "ERROR"
    }
...
Write-Log "* UseCsvForDuplicates = $UseCsvForDuplicates"
...
if ($FixDuplicates) {
    if ($UseCsvForDuplicates -and (Test-Path $CsvDuplicatePath)) {
        Import-DuplicateSecrets -CsvPath $CsvDuplicatePath -AuthToken $AuthToken
    }
    else {
        Write-Log "Running API discovery for duplicate secrets..."
        Find-DuplicateSecrets -AuthToken $AuthToken
    }
}

Write-Log "== Cleanup completed =="
Write-Log "* Total secrets updated: $($global:SecretUpdates.Count)"
Write-Log "* Total whitespace-fixed secrets: $($global:WhitespaceSecretUpdates.Count)"