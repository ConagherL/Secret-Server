...
# Define the log file path (now in the output directory)
$logFilePath = Join-Path $outputDirectory 'Migration_Log.txt'
...
function Process-Reports {
    ...
    foreach ($reportId in $reportIds) {
        ...
        # Include dualControlApproval if present
        if ($reportData.PSObject.Properties.Name -contains 'dualControlApproval') {
            if ($reportData.dualControlApproval) {
                $body.Add('dualControlApproval', $reportData.dualControlApproval)
            }
        }
